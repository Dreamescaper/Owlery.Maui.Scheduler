...
An agent that never appears in `maui devflow list` has often started fine and simply failed to
register. Check before assuming the app is broken:

```bash
adb logcat -d | grep -i devflow               # "Agent started on port 9223" means it is up
adb forward tcp:9223 tcp:9223
maui devflow --agent-port 9223 agent status   # talk to it directly, bypassing the broker
```

Every subsequent command needs the same `--agent-port 9223`. With an iOS agent also connected, the
broker answers as *that* one — `agent status` reporting `"platform": "iOS"` while you are targeting
Android is the tell.
...
**A Debug APK installed with plain `adb install` will crash on launch.** Debug builds default to
Fast Deployment (`EmbedAssembliesIntoApk=false`): the managed assemblies are *not* inside the APK,
they are pushed separately to `files/.__override__/<abi>/` by the MSBuild deploy target. `adb install`
skips that step, so the process aborts in native code before any managed code runs:
...
F monodroid: Abort at monodroid-glue.cc:757:3
```

There is no managed stack trace, and `adb logcat | grep AndroidRuntime` finds nothing, which makes it
look like an app bug rather than a deployment one. Build a self-contained APK instead:

```bash
dotnet build -f net10.0-android -p:EmbedAssembliesIntoApk=true
adb install -r bin/Debug/net10.0-android/<pkg>-Signed.apk
```

Uninstall first if a broken Fast Deployment install is already on the device — but note that
uninstalling **wipes app data**, so anything behind a login has to be signed in again.
...
### Launching without `-t:Run`

`-t:Run` shells out through MSBuild and breaks when the SDK path contains a space (see
`ios-and-mac.md` — same root cause, different tool). Launch by explicit activity instead; the class
name is CRC-mangled, so resolve it rather than guessing:

```bash
adb shell cmd package resolve-activity --brief -c android.intent.category.LAUNCHER <pkg> | tail -1
adb shell am start -n <pkg>/<resolved.MainActivity>
adb shell pidof <pkg>                        # empty means it died on startup
```
