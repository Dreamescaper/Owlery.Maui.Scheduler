...
**When `-t:Run` fails with exit code 127**, the SDK path contains a space. `-t:Run` shells out to
`mlaunch` through an unquoted generated command, so a path such as
`~/Library/Application Support/dotnetup/dotnet` breaks it:

```
/var/folders/.../tmp....exec.cmd: line 2: /Users/you/Library/Application: No such file or directory
error MSB3073: The command "..../mlaunch --launchsim ..." exited with code 127.
```

Nothing is wrong with the build — only the launch step. Build and deploy in separate steps instead:

```bash
dotnet build -f net10.0-ios
xcrun simctl terminate $UDID <bundle-id>      # otherwise launch reuses the old process
xcrun simctl install $UDID bin/Debug/net10.0-ios/iossimulator-arm64/<App>.app
xcrun simctl launch $UDID <bundle-id>
```

Unlike Android, an iOS Debug build is self-contained, so `simctl install` needs no extra flag, and
installing over an existing app **keeps its data** — logins survive.

Note the split for the `maui` CLI itself: it cannot find .NET unless `DOTNET_ROOT` is set, while
`dotnet` must not have it set, for the quoting reason above:

```bash
DOTNET_ROOT="/Users/you/Library/Application Support/dotnetup/dotnet" maui devflow ui screenshot
```
