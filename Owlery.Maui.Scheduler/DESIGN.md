...
Month and agenda views need different surfaces, not different day counts (sections 14 and 21). What
...
small price, and it means the header-mirroring of section 8 applies unchanged in both paging modes.
...
platform scroll view in section 19. The simpler surface is one table of rows, one vertical scroll
...
horizontal pager already does in section 19); that is deferred for now rather than taken lightly.
...
`PagingScrollView`, whose handler owns its frame (section 19), so a transform written there is
...
one section 19 keeps teaching: taking over a handler takes over everything it did, and a view inside
...
in section 19); that is why the range is deliberately left unbounded rather than trimmed.
...
upward-scroll correction wobble discussed in section 14, and the sample now avoids it by supplying a
